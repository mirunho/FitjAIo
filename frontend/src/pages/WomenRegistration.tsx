...
export default function WomenRegistration() {
  ...
  const scheduleDays = SCHEDULE.map((s) => ({
    ...s,
    date: addDays(monday, s.jsDay - 1),
  }));
  ...
  const loadAll = useCallback(async () => {
    ...
    for (const s of SCHEDULE) {
      const d = addDays(mon, s.jsDay - 1);
      for (const slot of s.slots) pairs.push([slot.classType, toDateStr(d)]);
    }
    await Promise.all(
      pairs.map(async ([classType, classDate]) => {
        const key = `${classType}::${classDate}`;
        try {
          const res = await getRegistrations(classType, classDate);
          setRegMap((prev) => ({
            ...prev,
            [key]: { list: res.data.registrations, limit: res.data.limit },
          }));
        } catch {
          /* ignore */
        }
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await createRegistration({
        class_type: modal.classType,
        class_date: modal.classDate,
        name: form.name.trim(),
        phone: form.phone.trim(),
      });
      setSuccessMsg(
        res.data.status === "waitlist"
          ? "Dodano na listę rezerwową!"
          : "Zapisano na zajęcia!"
      );
      await loadAll();
          ...
          return (
            <div key={jsDay} className="reg-day-col">
              ...
              {slots.map((slot) => {
                ...
                const spotsLeft = limit - registered.length;
                const isFull = spotsLeft <= 0;

                const spotsText = isFull
                  ? waitlist.length > 0
                    ? `Brak miejsc · rezerwowa: ${waitlist.length} os.`
                    : "Brak miejsc"
                  : `${spotsLeft} z ${limit} ${spotsLeft === 1 ? "miejsce wolne" : spotsLeft < 5 ? "miejsca wolne" : "miejsc wolnych"}`;

                return (
                  <div key={slot.classType} className="reg-slot">
                    <div className="reg-slot-top">
                      <div className="reg-slot-info">
                        <div className="reg-slot-time">{slot.time} – {slot.endTime}</div>
                        <div className="reg-slot-name">{slot.classType}</div>
                    ...
                    <div className="reg-spots-row">
                      <div className="reg-bar">
                        <div
                          className={`reg-bar-fill${isFull ? " reg-bar-full" : ""}`}
                          style={{ width: `${Math.min(100, (registered.length / limit) * 100)}%` }}
                        />
                      </div>
